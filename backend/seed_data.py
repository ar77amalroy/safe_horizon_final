...
for index, row in df.iterrows():
    try:
        ...
        historical_accident = models.Accident(
            user_email="system@safehorizon.com", # Tag as a system import
            latitude=str(row['latitude']),
            longitude=str(row['longitude']),
            severity=str(row['severity']), # Will now perfectly insert "Minor", "Major", or "Critical"
            description="Historical Data Import 2023",
            accident_datetime="2023-01-01 12:00 PM",
            status="approved" # Automatically approve so the AI can use it immediately
        )
        db.add(historical_accident)
        inserted_count += 1
        
        # Save to database in batches of 500 to save memory and prevent crashing
